			...
			success: function(data) {
				...
				if (data.errors != null && !jQuery.isEmptyObject(data.errors)) {
					for (var error of data.errors['body']) {
						$('div#comment_error').append('<div class="alert-danger" role="alert">' + error + '</div>');
					}
				}else {
					$('dib#comment_error').html('<div class="alert alert-success" role="alert">Комментарий добавлен</div>');
