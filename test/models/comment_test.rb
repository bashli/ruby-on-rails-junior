...
class CommentTest < ActiveSupport::TestCase
  ...
  def setup
  	...
  	create_list(:comment, 2, paremt_id: @comments[0].id, post: @post)
  	create_list(:comment, 3, paremt_id: @comments[1].id, post: @post)  	
  end
   ...
   test "fetch first level comments for post" do 
    ...
    test "fetch replies for post" do 
  		replies = Comments.replies_for_post(@post.id)
